...
public class DiningPhilosophersDynamic
{
	/*
	 * ------------
	 * Data members
	...
	public static void main(String[] argv)
	{
		try
		{
			System.out.println("Task 5 and Task 4:");
			...
			if(objStr.matches("[A-Za-z]+[0-9]*||[0-9]*[A-Za-z]+")) {
				...
				ArrayList<DynamicPhilosopher> philosopherArrList = new ArrayList<DynamicPhilosopher>(iPhilosophers);
				//OrderedPhilosopher OrderedaoPhilosophers[] = new OrderedPhilosopher[iPhilosophers];

				// Let 'em sit down
				for(int j = 0; j < iPhilosophers; j++)
				{
					philosopherArrList.add(j, new DynamicPhilosopher(j, philosopherArrList));
					philosopherArrList.get(j).start();
				}
				...
				for(int j = 0; j < iPhilosophers; j++) {
					...
					if(flag)
					{
						philosopherArrList.get(j).newJoin();
						// ...
					}
				}
